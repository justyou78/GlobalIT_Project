...
<%
	...
	count  = dao.noticeCount();	//공지사항 글 수 알려줌
	if(count > 0){
...
<body>
	<div class="container">
	<h1 class="text-center">공지사항</h1><br/>
	<button style="margin-bottom: 10px;" type="button" class="btn btn-primary float-right" 
	onclick="document.location.href='header.jsp?pgName=noticeWrite'">글쓰기</button>
	 <br/>
<%	if(count == 0){		%>
...
<%
		for(int i = 0; i < noticeList.size(); i++){
			BoardVO vo = (BoardVO)noticeList.get(i);	//vo에 담겨진 데이터 뽑아오기
%>		
		<tbody>
			<tr>
				<th scope="row"><%= number-- %></th> <!-- 게시글 번호 역순으로 -->
				<td><a href="header.jsp?pgName=noticeView&num=<%=vo.getNum() %>&pageNum=<%=currentPage %>"> <%= vo.getTitle() %></a></td>
				<td><%= vo.getId() %></td>
				<td><%= vo.getTime() %></td>
				<%if(id.equals("admin")){%>
               <td><button onclick="location='../board/boardAdminPro.jsp?num=<%= vo.getNum() %>&pgName=notice'">삭제</button></td>
            <%} %>
			</tr>
		</tbody>
	<%	}	//for %>
...
<%
	...
	System.out.println("페이지 카운트"+pageCount);
		...
		<%
		if(startPage > 10){		%>
			<a href="header.jsp?pgNmae=notice&pageNum=<%=startPage - 9 %>">[이전]</a>
<%		}
		for(int i = startPage; i <= endPage; i++){	%>
			<a href="header.jsp?pgName=notice&pageNum=<%=i %>">[<%= i %>]</a>
<%		}
		if(endPage < pageCount){	%>	
			<a href="header.jsp?pgName=notice&pageNum=<%=startPage + 10 %>">[다음]</a>	
<%		}
	}
%>
		</div>
